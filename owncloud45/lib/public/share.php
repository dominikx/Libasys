...
// 			return self::shareItem($itemType, $itemSource, self::SHARE_TYPE_EMAIL, $details['EMAIL'], $permissions);
				...
				if (isset($targets[$row[$column]])) {
					...
					if ($items[$id]['uid_owner'] == $row['uid_owner']) {
						...
						if (~(int)$items[$id]['permissions'] & self::PERMISSION_SHARE && (int)$row['permissions'] & self::PERMISSION_SHARE) {
                         ...
                         // $items[$id]['expiration'] =$row['expiration'];
	...
	*/
    ...
    public static function getAllSharesUser(){
       ...
       // link = parent.location.protocol+'//'+location.host+OC.linkTo('', 'public.php')+'?service=files&'+$('tr[data-id='+String(itemSource)+']').attr('data-type')+'='+file;
         ...
         $SQL="SELECT * FROM  `*PREFIX*share` WHERE uid_owner='".\OC_User::getUser()."'  ";
             ...
             foreach($shareInfo as $share){
                ...
                if($share['share_type']==1){
                 ...
                 if($share['share_type']==3){
                     ...
                     'shareType'=>'Link',
                     'shareName'=>'Guest',
                     'link'=>\OC_Helper::linkToPublic('files').'&'.$itemTypeChoose.'=/'.$share['uid_owner'].'/files'.$share['file_target'],
                      'name'=>$share['file_target'],
                      'date'=>'Gültig bis: '.$EXPDATE
                      );
                 }
