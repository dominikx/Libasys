...
class OC_Util {
	...
	*/
    ...
    public static function getAllApps(){
           ...
           $prepareArray=self::$appsArray;
           $returnArray=array();
           foreach($prepareArray as $info){
               if(!stristr($info,'files_') && !stristr($info,'admin_') && $info!='themes_manager' && $info!='user_migrate') $returnArray[$info]=$info;
           }
    ...
    public static function checkRightsApps(){
        
        $GETGROUPS=OC_GROUP::getGroups();
        
        
        $SQL="SELECT a.appid,ga.gid,ga.access  FROM *PREFIX*appconfig a  
                    LEFT JOIN *PREFIX*groups_apps_access ga ON a.appid=ga.appid
                    WHERE a.configkey='enabled' AND a.configvalue='yes'
                    ORDER BY a.appid ASC, ga.gid ASC
        ";
        ...
        while( $row = $result->fetchRow()){
            $dataInfo[$row['gid'].'#'.$row['appid']] =$row;
           
          
          if(!isset($dataApp[$row['appid']])) {
               $dataApp[$row['appid']]=$row['appid'];
            
          }
            
        }
        if($dataInfo!=''){
            self::$appsArray=$dataApp;
            
             foreach($GETGROUPS as $groupInfo){
                 
                 if($groupInfo!='admin'){   
                    foreach($dataApp as $info){
                           
                           $baccess=1;
                           if(isset($dataInfo[$groupInfo.'#'.$info]['access'])) $baccess=$dataInfo[$groupInfo.'#'.$info]['access'];
                           
                             $GROUPAPPACCESS[$groupInfo][]=array(
                                'gid'=>$groupInfo,
                                'appid'=>$info,
                                'access'=>$baccess
                        ...
                        // print $info;
                         ...
                         // if($groupInfo!=$info['gid']) $bSetter=false;
                          //if($groupInfo==$info['gid']) $bSetter=true;
                        
                         //if($groupInfo==$info['gid'] || $info['gid']==''){
                             /*
                            $GROUPAPPACCESS[$groupInfo][]=array(
                                'gid'=>$info['gid'],
                                'appid'=>$info['appid'],
