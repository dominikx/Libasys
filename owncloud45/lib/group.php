...
class OC_Group {
    ...
    public static function groupAppRights($APPID){
       
          if(OC_User::getUser()!=''){
          $GIDARRAY=OC_Group::getUserGroups(OC_User::getUser());
          $SQLWHERE="";
          $bAdmin=false;
          foreach($GIDARRAY as $gid){
            
              if($gid=='admin') $bAdmin=true;
              if($SQLWHERE=='') $SQLWHERE.="AND (gid='".$gid."' ";
              else $SQLWHERE.="OR gid='".$gid."' ";
          }
          $SQLWHERE.=")";
         
        if($bAdmin) return true;
     
        if(!$bAdmin && $SQLWHERE!='') {
             $SQL="SELECT access FROM *PREFIX*groups_apps_access WHERE appid='".$APPID."' ".$SQLWHERE." ";
