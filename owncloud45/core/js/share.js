			...
			if (data.shares) {
				$.each(data.shares, function(index, share) {
					if (share.share_type == OC.Share.SHARE_TYPE_LINK) {
						...
						if(share.expiration){
								OC.Share.showExpDate(itemSource, share.expiration);
						}
					} else {
						if (share.collection) {
...
// 							}
	...
	addShareWith:function(shareType, shareWith, permissions, possiblePermissions, collection) {
	 ...
	 showExpDate:function(item_source,expDate){
	...
	showLink:function(itemSource, password) {
		...
		var link = parent.location.protocol+'//'+location.host+OC.linkTo('', 'public.php')+'?service=files&'+$('tr[data-id='+String(itemSource)+']').attr('data-type')+'='+file;
		$('#linkText').val(link);
		$('#linkText').show('blind');
		$('#showPassword').show();
		if (password != null) {
