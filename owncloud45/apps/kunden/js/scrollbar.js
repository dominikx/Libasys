...
var ScrollBar = new Class({
	...
	initialize : function(scroller, slider, knob, options) {
		...
		if (this.options.knob.duration || this.options.knob.transition)
			this.knobFx = this.knob.get('tween');
	...
	clickedElement : function(event) {
		if (this.knobFx && event.target === this.knob)
			this.knobFx.cancel();
		this.parent(event);
	},
	...
	set : function(pos) {
		if (typeOf(pos) === 'element')
			pos = pos.getPosition(this.scroller)[this.axis] * this.ratioScroll;
		this.parent(pos);
	}
}); 