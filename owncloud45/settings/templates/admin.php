...
<form id="appmanager">
    <fieldset class="personalblock">
        <label>Default App</label><br />
        <select id="app_switcher">
            
            <?php foreach($_['apps'] as $key => $value): ?>
            <option value='<?php print(strtolower($key)); ?>'<?php print($_['current1']==strtolower($key)?' selected':''); ?>><?php print($value); ?></option>
            <?php endforeach;?>
        </select>
    </fieldset>
</form>
<div class="personalblock">
    <legend> Berechtigungen Gruppe => Apps</legend><br />
      <form name="chGroupForm" id="chGroupForm" method="post" action=" ">
     <?php if(is_array($_['grouprights'])) {$OLDKEY=''; foreach($_['grouprights'] as $KEY => $groupInfo):
           
         print '<span style="width:90%; float:left; display:block;"><b>'.$KEY.'</b><br />';
            foreach($groupInfo as $info):
                  $GID=$info['gid'];
                  $APPID=$info['appid'];
                   $checked='';
                   $oldchecked='';
                    if($KEY==$GID && $info['access'] || $KEY=='admin') {$oldchecked='on'; $checked='checked';} 
                    
             ?>
               <input type="hidden" name="old_<?php echo $KEY; ?>##<?php echo $APPID; ?>" value="<?php echo $oldchecked; ?>" />
             <span style="width:200px;float:left;display:block;"> <input type="checkbox" name="<?php echo $KEY; ?>##<?php echo $APPID; ?>" <?php echo $checked; ?> /> <?php echo $APPID; ?></span>
           <?php
                  
            endforeach;
             print '</span><br style="clear:both;" /><br />';
        endforeach;
        print '<br style="clear:both;" /> <input type="button" id="changeGroups" value="speichern" />';
